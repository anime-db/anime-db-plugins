...
│       ├── assets/            # статика (CSS/JS/картинки/шрифты), которую хост может подключить
│       └── translations/      # каталоги переводов — обязательны у translation
│           └── native/        # необязательный каталог переводов нативного слоя приложения
...
### Каталог `translations/native/<locale>.json`

Плагин типа `translation` может дополнительно везти `translations/native/<locale>.json` —
перевод нативного слоя приложения (сплэш-экран, меню трея, диалоги отказа запуска), который
живёт отдельно от каталога `translations/messages.<locale>.yaml`, описанного выше. Каталог
опциональный: у большинства языковых пакетов его нет, и это нормально.

- Формат — плоский JSON-объект `строка → строка`. Вложенность запрещена: нативный слой
  обращается к ключу простым `catalog[key]` и не найдёт вложенное значение.
- Формат именно JSON, а не YAML: у нативного слоя нет рантайм-зависимостей и YAML-парсера.
- Каждая локаль, для которой есть файл в `translations/native/`, обязана быть объявлена в
  `locales` манифеста. Обратное не требуется: локаль в `locales` без соответствующего файла
  в `translations/native/` — валидна, поскольку сам каталог необязателен.
- Действуют те же контентные правила, что и для `messages.<locale>.yaml`: пустое значение
  запрещено, плейсхолдеры — только вида `%name%`, символ `|` запрещён. Паритет ключей и
  плейсхолдеров требуется между локалями, но только внутри самого `translations/native/`.
- Ключи `translations/native/` не входят в `translation_keys_count` — это поле по-прежнему
  считает только листовые ключи `translations/messages.<locale>.yaml`.
- Пространство имён ключей `translations/native/` не пересекается с `messages`: совпадение
  имени ключа между ними не является ошибкой.

Формат — `translations/native/<locale>.json` (подкаталог), а не плоский файл
`translations/native.<locale>.json`: хост отдаёт каталог `translations/` каждого плагина в
пути переводов Symfony, и имя файла с двумя точками было бы подхвачено как отдельный домен
переводов, которым оно не является.

### Каталог `assets/` и поле манифеста `ui`

Плагин со своей вёрсткой (стили страницы настроек, скрипт виджета и т.п.) кладёт эти файлы
в `assets/` и объявляет их в `manifest.json`:

```json
"ui": { "css": ["assets/carousel.css"], "js": ["assets/settings.js"] }
```

Хост подключает перечисленные там файлы там, где сам рисует шелл страницы (страница
настроек плагина, страница с его виджетами) — подключать их из разметки плагина в этих
местах не нужно. На собственных страницах плагина и в ответах его собственных маршрутов
(например, страница результата OAuth со своим `<head>`) шелл хоста не участвует, и теги
`<link>`/`<script>` там ставит сам плагин. Поле
`ui` доступно с `anime-db/plugin-contracts` `v0.19`, поэтому плагин, объявивший `ui`,
обязан пинить `require.plugin-contracts` диапазоном, покрывающим `0.19` (например
`^0.19`) — иначе он обещает хост, который это поле ещё не умеет читать.

Картинки в `ui` не объявляются: разметка плагина подключает их сама через Twig-функцию
хоста `plugin_asset()`, либо CSS плагина ссылается на них относительным `url(...)`.

Любой файл в `assets/` (объявлен он в `ui` или нет) обязан иметь одно из расширений,
которые хост реально отдаёт: `.css`, `.js`, `.svg`, `.png`, `.webp`, `.woff2`. Файл с
другим расширением всё равно уедет в ZIP плагина (`assets/` не входит в список
исключений сборщика), но ни один маршрут хоста его не отдаст — мёртвый груз.
`tools/src/PluginValidator.php` проверяет и существование объявленных в `ui` файлов
внутри каталога плагина, и то, что их путь не выходит за пределы `assets/`, и
аллоу-лист расширений для всего содержимого `assets/` — но не содержимое самих файлов
(ни линтера JS, ни разбора CSS).

Виджеты плагина приезжают в страницу хоста уже после её загрузки (через htmx-свопы), а
не присутствуют в ней при `DOMContentLoaded`. JS плагина обязан это учитывать: обработчики
вешаются делегированием на `document` (`document.addEventListener('click', e => { if
(e.target.matches('.my-widget button')) { ... } })`) или по событию `htmx:load`, а не
одноразовым обходом DOM на `DOMContentLoaded` — такой обход просто не увидит виджет,
подставленный позже.
