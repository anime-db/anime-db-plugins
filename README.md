...
`plugins-registry.json` (третий аргумент). Если этот аргумент вовсе не передан —
`sequence` стартует с `1` (первая генерация). Если аргумент передан, но по этому пути
файла нет, сборка завершается с ошибкой, а не молча стартует счётчик заново. Полей
