...
раздачи на зеркале, **абсолютный путь** — с ведущим `/`), `protocol` (`ftps` — по
умолчанию и предпочтительно, `ftp` — фолбэк для хостинга без FTPS).

> **Замечание безопасности (FTPS).** FTPS через `ext-ftp` шифрует канал, но **не
> проверяет сертификат сервера** — защита от пассивного прослушивания, но не от
> активного MITM (который мог бы украсть write-креды зеркала). Принято как остаточный
> риск: зеркало — недоверенный транспорт, целостность держит подпись реестра (клиент
> пинит `sha256`), поэтому компрометация зеркала бьёт по доступности, не по целостности.
> Нужна настоящая проверка сертификата — другой транспорт (curl/SFTP).

`tools/push-mirror-assets.php` читает этот JSON из
