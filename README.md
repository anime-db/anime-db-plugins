...
│       ├── composer.json      # autoload.psr-4 для src/ (обязателен, хост грузит классы через него)
...
Плагин — это `manifest.json` + `composer.json` (только `autoload.psr-4` — хост
читает по нему namespace, не запускает сам composer) + `src/*.php`, без собственного
DI-конфига, без класса бандла и без тегов: всё это хост-приложение настраивает само
(регистрация сервисов, тегирование по контрактным интерфейсам, автозагрузка
изолированного namespace). Контракт (`anime-db/plugin-contracts`) в рантайме предоставляет хост,
...
# в src/, соответствие composer.json autoload.psr-4, синтаксис PHP. Код возврата 0 —
# плагин валиден, иначе печатает список проблем.
